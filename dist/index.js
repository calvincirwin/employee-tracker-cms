...
const inquirer_1 = __importDefault(require("inquirer")); // For user interaction
const connection_1 = __importDefault(require("./connection")); // Your database connection pool
// Function to view all departments
async function viewDepartments() {
    try {
        const result = await connection_1.default.query('SELECT * FROM department');
...
async function addDepartment() {
    ...
    try {
        await connection_1.default.query('INSERT INTO department (name) VALUES ($1)', [name]);
...
async function addRole() {
    ...
    try {
        await connection_1.default.query('INSERT INTO role (title, salary, department_id) VALUES ($1, $2, $3)', [title, salary, department_id]);
...
async function updateEmployeeRole() {
    const { employee_id, new_role_id } = await inquirer_1.default.prompt([
        {
            type: 'input',
            name: 'employee_id',
    ...
    try {
        await connection_1.default.query('UPDATE employee SET role_id = $1 WHERE id = $2', [new_role_id, employee_id]);
...
async function mainMenu() {
    ...
    switch (action) {
        ...
        case 'Update Employee Role':
            ...
            break;
        case 'Exit':
            await connection_1.default.end(); // Close the database connection when exiting
            console.log('Goodbye!');
            process.exit(0);
    }
